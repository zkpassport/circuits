...
describe("outer proof", () => {
  ...
  test(
    ...
    async () => {
      ...
      // Facematch proof generated a day earlier with zero scope/subscope and hidden
      // sensitive inputs, like the enrollment proof the app caches
      ...
      const facematchParamCommitment = getParameterCommitmentFromDisclosureProof(facematchProof)
      ...
      expect(currentDate.getTime()).toEqual(nowTimestamp * 1000)
      const nullifier = getNullifierFromOuterProof(proof)
      expect(nullifier).toEqual(getNullifierFromDisclosureProof(ageProof))
      const paramCommitmentsFromProof = getParamCommitmentsFromOuterProof(proof)
      expect(ageParamCommitment).toEqual(paramCommitmentsFromProof[0])
      expect(facematchParamCommitment).toEqual(paramCommitmentsFromProof[1])
      await outerProofCircuit.destroy()

      // A facematch proof bound to another service must not pass as scope-less
      const otherServiceFacematchInputs = await getFacematchCircuitInputs(
        helper.passport as any,
        facematchQuery,
        INTEGRITY_TO_DISCLOSURE_SALTS,
        0n,
        getServiceScopeHash("other-service.com"),
        0n,
        nowTimestamp,
        true,
      )
      const otherServiceFacematchCircuit = Circuit.from("facematch_ios")
      const otherServiceFacematchProof = await otherServiceFacematchCircuit.prove(
        { ...otherServiceFacematchInputs, ...FIXTURES_FACEMATCH.ios_regular_mode_dev },
        {
          useCli: true,
          recursive: true,
          circuitName: "facematch_ios",
      ...
      await otherServiceFacematchCircuit.destroy()
      const rejectedOuterCircuit = Circuit.from("outer_count_5")
      const rejectedInputs = {
        ...inputs,
        disclosure_proofs: [
          inputs.disclosure_proofs[0],
          {
            ...inputs.disclosure_proofs[1],
            proof: otherServiceFacematchProof.proof as string[],
            public_inputs: otherServiceFacematchProof.publicInputs as string[],
          },
        ],
      }
      await expect(
        rejectedOuterCircuit.prove(rejectedInputs, {
          useCli: true,
          circuitName: "outer_count_5",
          recursive: true,
        }),
      ).rejects.toThrow("Disclosure proof service scope must be either 0 or the expected service scope")
      await rejectedOuterCircuit.destroy()

      // A tampered top-level param commitment must fail the param-commitment binding
      const tamperedParamCommitmentCircuit = Circuit.from("outer_count_5")
      const tamperedParamCommitmentInputs = {
        ...inputs,
        param_commitments: [inputs.param_commitments[0], "0x1"],
      }
      await expect(
        tamperedParamCommitmentCircuit.prove(tamperedParamCommitmentInputs, {
          useCli: true,
          circuitName: "outer_count_5",
          recursive: true,
        }),
      ).rejects.toThrow("Disclosure proof param commitment must match the expected param commitment")
      await tamperedParamCommitmentCircuit.destroy()
    },
    60000 * 5,
  )
})
