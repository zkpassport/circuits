...
import { createMockOPRFEvaluator, evaluateOPRF } from "@zkpassport/utils"
...
describe("subcircuits - RSA PKCS", () => {
  ...
  describe("salted OPRF nullifier", () => {
    ...
    test("should produce valid proof with OPRF-based salted nullifier", async () => {
      ...
      // Get the dg2_hash from the circuit inputs and compute OPRF with dg2_hash as the input
      const dg2Hash = BigInt(inputs.salted_dg2_hash.value as string)
      const { evaluator } = createMockOPRFEvaluator()
      const { oprfProof, oprfOutput, publicKey } = await evaluateOPRF(dg2Hash, evaluator)
