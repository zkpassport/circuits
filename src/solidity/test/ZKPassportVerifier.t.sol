...
contract ZKPassportVerifierTest is TestUtils {
    ...
    string constant ALL_SUBPROOFS_PUBLIC_INPUTS_PATH = "./test/fixtures/all_subproofs_public_inputs.json";
    string constant ALL_SUBPROOFS_COMMITTED_INPUTS_PATH = "./test/fixtures/all_subproofs_committed_inputs.hex";

    bytes32 constant VKEY_HASH = bytes32(uint256(0x04b98c6f867d6a7f86d514b72c3be8f41b7aa6f49fdc17514c9f9f0a2ac3ef9a));
    ...
    uint256 constant CURRENT_DATE = 1756064015;
    uint256 constant PROOF_GENERATION_DATE = 1756063905;
    ...
    bytes memory data = zkPassportVerifier.getBindProofInputs(
      ...
      committedInputCounts
        ...
        (address senderAddress, uint256 chainId, string memory customData) = zkPassportVerifier.getBoundData(data);
    ...
    function test_VerifyAllSubproofsProof_2() public {
      ...
      (
        ...
        uint256 maxDateBirthDate
          ...
          ProofType.BIRTHDATE
            ...
            assertEq(maxDateBirthDate, 3965052705);
        }
      ...
      (
        ...
        }
        {
            ...
            string[] memory countryList = zkPassportVerifier.getCountryProofInputs(
                committedInputs, committedInputCounts, ProofType.ISSUING_COUNTRY_INCLUSION
            );
            gasUsed = vm.stopSnapshotGas();
