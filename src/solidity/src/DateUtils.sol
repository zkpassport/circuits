...
library DateUtils {
    ...
    /**
     * @dev Converts a date array to a UNIX timestamp
     * @param date The date in this format: [Y1, Y2, Y3, Y4, M1, M2, D1, D2]
     * @return The UNIX timestamp in seconds
     */
    function getTimestampFromDate(bytes32[] memory date) internal pure returns (uint256) {
     ...
     * @param dateArray The date array in this format: [Y1, Y2, Y3, Y4, M1, M2, D1, D2]
     * @param validityPeriodInDays The validity period in days
     * @return True if the date is valid and within the validity period
     */
    function isDateValid(bytes32[] memory dateArray, uint256 validityPeriodInDays) internal view returns (bool) {
        uint256 timestamp = getTimestampFromDate(dateArray);
