...
contract ZKPassportSubVerifier {
  ...
  function _validateCertificateRoot(IRootRegistry _rootRegistry, bytes32 certificateRoot, uint256 timestamp) internal view {
    require(
  ...
  function _validateCircuitRoot(IRootRegistry _rootRegistry, bytes32 circuitRoot, uint256 timestamp) internal view {
    require(
      _rootRegistry.isRootValid(CIRCUIT_REGISTRY_ID, circuitRoot, timestamp),
  ...
  function verify(
  ...
  ) external view whenNotPaused onlyRootVerifier returns (bool isValid, bytes32 uniqueIdentifier) {
    ...
    _validateCertificateRoot(rootRegistry, params.proofVerificationData.publicInputs[PublicInput.CERTIFICATE_REGISTRY_ROOT_INDEX], currentTimestamp);

    // Validate circuit registry root
    _validateCircuitRoot(rootRegistry, params.proofVerificationData.publicInputs[PublicInput.CIRCUIT_REGISTRY_ROOT_INDEX], currentTimestamp);
    ...
    require(_checkDateValidity(currentTimestamp, params.serviceConfig.validityPeriodInSeconds), "The proof was generated outside the validity period");
    ...
    require(_verifyScopes(params.proofVerificationData.publicInputs, params.serviceConfig.domain, params.serviceConfig.scope), "Invalid domain or scope");
    ...
    isValid = IProofVerifier(verifier).verify(params.proofVerificationData.proof, params.proofVerificationData.publicInputs);
